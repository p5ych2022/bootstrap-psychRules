...
当项目需要一套可持续维护的上下文系统，并且希望后续线程持续复用这些信息时，使用这个 skill。

The skill itself should be written in bilingual format: each English paragraph is immediately followed by its Chinese translation.
这个 skill 本体必须采用中英双语对照格式：每一段英文后面紧跟对应中文翻译。
...
Update `.psychRules/rules.md` with startup commands, dependencies, warnings, review hotspots, commit-memory policy, and language rules.
把启动命令、依赖、警告事项、审查重点、commit-memory 同步规则以及语言规则写进 `.psychRules/rules.md`。
...
如果用户要求为某次代码改动创建 memory，但还没有要求 commit，则补一句简短确认：是否要顺便创建对应的 git commit。
